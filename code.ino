...
/* Hex codes from the IR Remote Control LED618 */
...
/* ------ Global Variables ----------------------------------------------------------------------- */
int      color       = BLACK; /* Stores the current led color, in range [0,16] */
int      color_back  = BLACK; /* Stores the last active led color, due to color variable */
int      bright      = 0; /* Keeps track of the current led brightness, in range [0,5] */
...
int16_t green = 0x00; 
int16_t blue  = 0x00;
int64_t active_color;
int64_t  last_color;

unsigned long testc;

unsigned long previousMillis = 0;   /* Stores last time LED faded to another color */
unsigned long interval       = 0;
const    long fade_interval  = 3000;    /* Interval at which to fade (milliseconds) */
const    long flash_interval = 1000;    /* Interval at which to flash (milliseconds) */
const    long smooth_interval = 100;    /* Interval at which to flash (milliseconds) */
/* Lookup table of RGB color values */
const unsigned long  colors[17][5] = { 
...
void setBrightness ( boolean direct ) {
  if ( powerState && !doFade && !doFlash && !doSmooth ) { /* Led's state is active */
...
void setColor ( int incolor ) {
  ...
  if ( powerState ) /* Verify that Led's state is active */
    setRGB( active_color = colors[ color = incolor ][bright] );
}

void setOnOff ( boolean input ) {
  if ( powerState && !input ) { /* Led is active */
    last_color = active_color;   /* Save led's state */ 
    setColor( BLACK );/* Set it Off */
...
void loop() {
  ...
  if ( irrecv.decode( &ir_results ) ) { /* Ir signal detected */
    ...
    switch ( ir_results.value ) {
      ...
      case SMOOTH:         if ( !doFade && !doFlash )   doSmooth = !doSmooth; break;
  ...
  if ( doFade || doFlash ) {
    ...
    if (currentMillis - previousMillis >= interval) {
      ...
      ( color !=  REBPURPLE ) ? ( color++ ) : ( color = WHITE );
      setRGB( active_color = colors[ color ][bright] );
    }
  }
 if ( doSmooth ) {
    unsigned long currentMillis = millis();
    ...
    if ( currentMillis - previousMillis >= interval ) {
      previousMillis = currentMillis; // save the last time you blinked the LED
      red =    ( (active_color >> 16) );
      green =   modVal((( active_color >> 8  ) & 0x00FF ));
      blue =   ((active_color & 0xFF));
      Serial.println(  "red" ); /* Use for debug */
      Serial.println(  red , HEX ); /* Use for debug */
      Serial.println(  "green" ); /* Use for debug */
      Serial.println(  green , HEX ); /* Use for debug */
      Serial.println(  "blue" ); /* Use for debug */
      Serial.println(  blue , HEX ); /* Use for debug */
      Serial.println(  "mixed" ); /* Use for debug */
      testc = ( ( red  & 0xFFFFFF) << 16 ) | ((green & 0xFFFFFF) << 8 ) | (blue);
      Serial.println(   testc, HEX ); /* Use for debug */
      setRGB( active_color = testc );
    }
  }
}
